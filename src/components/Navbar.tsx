  ...
  return (
    <header className="sticky top-0 z-50 bg-slate-950/90 backdrop-blur-md border-b border-cyan-500/20 text-slate-100 shadow-lg shadow-cyan-950/30">
      <div className="bg-gradient-to-r from-slate-950 via-cyan-950/40 to-slate-950 px-4 py-1 text-xs border-b border-cyan-500/10 flex items-center justify-between text-slate-400">
        <div className="flex items-center gap-4 min-w-0 overflow-x-auto whitespace-nowrap scrollbar-none">
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        <div onClick={() => setActiveTab('dashboard')} className="flex items-center gap-3 cursor-pointer group shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="font-bold text-lg tracking-wide text-white font-mono bg-gradient-to-r from-cyan-300 via-teal-200 to-purple-300 bg-clip-text text-transparent">PQ-RDL</h1>
              <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-purple-900/50 border border-purple-500/30 text-purple-300 font-mono font-semibold">Web 4.0</span>
            </div>
            <p className="text-[11px] text-slate-400 font-mono">Post-Quantum Conway Automaton Chain</p>
          </div>
        </div>

        {/* Desktop navigation: constrained width + horizontal scroll so tabs never push the wallet off-screen. */}
        <nav className="hidden lg:flex min-w-0 flex-1 items-center gap-1 bg-slate-900/80 p-1.5 rounded-xl border border-slate-800 overflow-x-auto overflow-y-hidden whitespace-nowrap scrollbar-thin scrollbar-track-slate-900 scrollbar-thumb-slate-700">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium font-mono transition-all shrink-0 ${
                  isActive
        ...
        <div className="flex items-center gap-2 shrink-0">
          {activeWallet ? (
            <button onClick={onOpenWalletModal} className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 border border-cyan-500/30 px-3 py-1.5 rounded-lg text-xs font-mono transition-all text-cyan-300">
              <div className="w-2 h-2 rounded-full bg-emerald-400"></div>
              <span className="hidden xl:inline">{activeWallet.algorithm}:</span>
              <span className="text-slate-300">{activeWallet.address.substring(0, 8)}...{activeWallet.address.slice(-6)}</span>
            </button>
          ) : (
            <button onClick={onOpenWalletModal} className="flex items-center gap-2 bg-gradient-to-r from-cyan-600 to-purple-600 hover:from-cyan-500 hover:to-purple-500 text-white px-3 py-2 rounded-lg text-xs font-medium font-mono shadow-md shadow-cyan-900/30 transition-all">
              <Wallet className="w-4 h-4" />
              <span className="hidden xl:inline">Create PQ Wallet</span>
              <span className="xl:hidden">PQ Wallet</span>
            </button>
          )}
        </div>
      </div>

      {/* Mobile/tablet navigation: horizontally scrollable, contained below the header. */}
      <div className="lg:hidden flex items-center gap-1 px-3 py-2 overflow-x-auto overflow-y-hidden bg-slate-900/90 border-t border-slate-800 whitespace-nowrap scrollbar-thin scrollbar-track-slate-900 scrollbar-thumb-slate-700">
        {navItems.map((item) => {
          const Icon = item.icon;
          const isActive = activeTab === item.id;
          return (
