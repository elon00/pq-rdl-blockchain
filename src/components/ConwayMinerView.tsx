...
export const ConwayMinerView: React.FC<ConwayMinerViewProps> = ({
  ...
  const handleMineBlock = async () => {
    setIsMining(true);
    setMiningStatus('Running local Conway proof simulation...');
    ...
    try {
      ...
      if (data.success && data.block) {
        setMiningStatus(`Local prototype block #${data.block.height} generated. This is not public-network settlement.`);
        ...
        setMiningStatus('Prototype block-generation API is disabled or unavailable.');
      }
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 p-5 rounded-2xl">
        <div>
          <div className="flex items-center gap-2 text-cyan-400 font-mono text-xs font-semibold">
            ...
            <span>Conway Proof Simulation</span>
          </div>
          <h2 className="text-xl sm:text-2xl font-bold font-mono text-white">
            Conway Automaton Block Experiment
          </h2>
          <p className="text-slate-400 text-xs">
            Draw cell patterns and evolve generations to explore the prototype proof function. This UI does not demonstrate public-network consensus.
          </p>
        </div>
        ...
        >
          {isMining ? (
            ...
            <>
              ...
              <span>Generate Local Prototype Block</span>
            </>
          )}
        </button>
      </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 space-y-3">
            <h3 className="text-sm font-bold font-mono text-white flex items-center gap-2">
              ...
              <span>Conway Preset Patterns</span>
            </h3>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-4 space-y-3 text-xs">
            <div className="flex items-center gap-2 text-amber-400 font-mono font-bold">
              ...
              <span>Prototype Proof Function</span>
            </div>
            <p className="text-slate-300 leading-relaxed font-mono text-[11px]">
              The prototype proof function hashes the initial cellular grid, evolves Conway B3/S23 rules, computes entropy, and can sign a locally generated result. This experiment is separate from independent consensus/security verification.
            </p>
          </div>
        </div>
      </div>
